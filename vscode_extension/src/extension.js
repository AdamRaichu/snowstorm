 ...
 */
async function findFileFromContent(base_directories, options, check_file) {
  let deprioritized_files = [];

  async function checkFile(path) {
    try {
      let content;
      if (options.read_file !== false) content = (await vscode.workspace.fs.readFile(path)).toString();
  ...
  let searchFolder = async (path) => {
    ...
    try {
      ...
      files = await vscode.workspace.fs.readDirectory(path);
    } catch (err) {
      files = [];
    }
    for (let fileInfo of files) {
      ...
      if (!options.filter_regex || options.filter_regex.exec(filePath)) {
        let new_path = path + "/" + filePath;
        if (!options.priority_regex || options.priority_regex.exec(filePath)) {
          // priority checking
          let result = await checkFile(new_path);
    ...
    if (options.recursive !== false) {
      for (let fileInfo of files) {
        ...
        let result = searchFolder(path + "/" + filePath);
        if (result) return result;
      }
    }
  };
  ...
  for (let path of deprioritized_files) {
    let result = await checkFile(path);
    if (result) return result;
  }
}

async function getParticleFileFromIdentifier(identifier, document_path) {
  let path_arr = document_path.split("/");
  let particle_index = path_arr.indexOf("particles");
  path_arr.splice(particle_index + 1);
  let filePath = path_arr.join("/");
  let name = identifier.split(":")[1];
  let files =
    (await findFileFromContent([filePath], { filter_regex: /\.json$/i, priority_regex: new RegExp(name, "i"), json: true }, (path, content) => {
...
class SnowstormEditorProvider {
    ...
    webviewPanel.webview.onDidReceiveMessage(async (e) => {
      switch (e.type) {
        ...
        case "save_texture": {
          ...
          let filePath = // Path.join(path_arr.join("/"), e.path.replace(/\.png$/, "") + ".png");
            vscode.Uri.joinPath(vscode.Uri.file(path_arr.join("/")), e.path.replace(/\.png$/, "") + ".png");

          await vscode.workspace.fs.writeFile(vscode.Uri.file(filePath), new Uint8Array(Buffer.from(e.content.split(",")[1], "base64")));
        ...
        case "request_particle_file": {
          let [match_path, match_content] = await getParticleFileFromIdentifier(e.identifier, document.fileName);
          if (match_content) {
            webviewPanel.webview.postMessage({
        ...
        case "open_particle_file_tab": {
          let [match_path] = await getParticleFileFromIdentifier(e.identifier, document.fileName);
          if (match_path) {
            (async function () {
        ...
        case "texture_autocomplete": {
          let json_path_arr = document.fileName.split("/");
          let json_particle_index = json_path_arr.indexOf("particles");
          json_path_arr.splice(json_particle_index);
          let directory_path = // Path.join(json_path_arr.join("/"), e.path);
            vscode.Uri.joinPath(vscode.Uri.file(json_path_arr.join("/")), e.path);

          let list = [];
          try {
            // list = fs.readdirSync(directory_path);
            var dirInfo = await vscode.workspace.fs.readDirectory(vscode.Uri.file(directory_path));
  ...
  getHtmlForWebview(webview) {
    const scriptUri = webview.asWebviewUri(vscode.Uri.parse(vscode.Uri.joinPath(this.context.extensionUri, "snowstorm", "app.js").toString()));
